#algorithm ENN with differential criterio Hamming Loss>Density
import pandas as pd
import numpy as np
...
def edited_nearest_neighbors(X, y, k=3, output_file=None):
    ...
    #     relevant_labels = y[neighbors]
    #     majority_class = np.bincount(relevant_labels.ravel()).argmax()  # Calculate majority class
    #     hamming_distance = hamming_loss(y[i], np.full_like(y[i], majority_class))
    #     print('hamming distance is:', hamming_distance)
    #
    #     # Check if hamming loss is 1, meaning different majority class
    #     if hamming_distance > 0.179:# if hamming is 1 dropped (differential criterio)
    ...
    for i, neighbors in enumerate(indices):
        hamming_loss_distances = [hamming_loss(y[i], y[n]) for n in neighbors]  # Exclude the current instance

        print(f'Hamming Loss distances with neighbors of instance {i}: {hamming_loss_distances}')

        # Check if two or more Hamming distances are equal to 1
        if sum(distance > 0.150  for distance in hamming_loss_distances) == 3:  # pano apo two instances jaccard distance 1 or >=2
            keep_indices[i] = False
...
print ('length of cleaned is:',length_cleaned_df_1)
...
print ('length of cleaned is:',length_cleaned_df_2)
...
print ('length of cleaned is:',length_cleaned_df_3)
...
print ('length of cleaned is:',length_cleaned_df_4)
...
print ('length of cleaned is:',length_cleaned_df_5)
