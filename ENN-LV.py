#algorithm ENN with differential criterio Levensthein distance>Cardinality/2
import pandas as pd
import numpy as np
...
def lvd(x, y):
    ...
    for i in range(1, m + 1):
        for j in range(1, n + 1):
            cost = 0 if x_list[i - 1] == y_list[j - 1] else 1
            dp[i][j] = min(
                dp[i - 1][j] + 1,  # Deletion
...
def edited_nearest_neighbors(X, y, k=5, output_file=None):
    ...
    for i, neighbors in enumerate(indices):
        Levenshtein_distances = [lvd(y[i], y[n]) for n in neighbors]  # Exclude the current instance

        print(f' Levenshtein distances with neighbors of instance {i}: {Levenshtein_distances}')

        # Check if two or more Jaccard distances are equal to 1
        if sum(distance > 20 for distance in Levenshtein_distances) >= 3:# pano apo two instances jaccard distance 1 or >=2
